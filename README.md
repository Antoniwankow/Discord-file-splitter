...
- 🌐 **Работает офлайн** — после первого запуска с интернетом приложение сохраняется и работает даже без соединения
- 🔄 **Удобное обновление** — при запуске загрузчика необходимо скачать страницу, после она сохраняется в кэш
...
2. **Манифест** — в репозитории лежит файл `manifest.json`, где указано имя актуального основного HTML-файла (например, `discord-file-splitter v1.6.1.html`).
3. **Загрузка** — при наличии интернета загрузчик читает манифест, скачивает указанный файл и сохраняет его в браузерный кэш (Cache API).
4. **Офлайн-режим** — если интернет отсутствует, загрузчик берёт сохранённую версию из кэша и запускает её.
5. **Удобное обновление** — при запуске с интернетом проверяется манифест, и если версия изменилась (или изменилось имя файла), загрузчик обновляет кэш.
6. **Без ручных действий** — пользователю не нужно скачивать новые ZIP-архивы, перезаписывать файлы или чистить кэш браузера. Один файл улучшает удобство взаимодействия с инструментом.

> **Для разработчиков:** Достаточно обновить файл в репозитории и изменить `entry` в `manifest.json` (если меняется имя), и все пользователи получат новую версию при попытке скачивания кэша.
...
1. **Через загрузчик (рекомендуется)** — откройте в браузере файл загрузчика `discord-file-splitter.html`, который при скачивании кэша подхватит актуальную версию. Загрузчик сохранит приложение, и в дальнейшем оно будет работать даже без интернета.
...
> 🔄 **Важный момент:** Если вы используете загрузчик, то при запуске с интернетом при нажатии на кнопку скачивания кэша, загрузчик проверяет наличие новой версии на GitHub и обновляет кэш. Вам больше не нужно следить за репозиторием, достаточно периодически нажимать кнопку "Скачать", чтобы проверять/обновлять новую версию.
